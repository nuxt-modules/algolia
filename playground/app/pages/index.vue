<template>
  <UContainer class="py-8 space-y-12">
        ...
        <UPageCard :ui="{ header: 'w-full border-bottom-1 border-muted' }">
          <template #header>
            ...
            <div class="flex items-center justify-between">
              <h3 class="text-lg font-semibold">Interactive Search</h3>
              <UBadge :label="`${searchResult?.nbHits || 0} results`" color="primary" />
            </div>
          </template>
          <template #body>
            <div class="space-y-4">
              <UInput
                ...
                @input="handleSearch"
              />
              <div v-if="searchResult?.hits?.length" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <UPageCard
    ...
    <section>
      <h2 class="text-2xl font-semibold text-primary mb-6 flex items-center gap-2">
        <UIcon name="i-ph-cloud-arrow-down" class="w-6 h-6" />
        useAsyncAlgoliaSearch - SSR Search
      </h2>
      <UPageCard :ui="{ header: 'w-full border-bottom-1 border-muted' }">
        <template #header>
          <div class="flex items-center justify-between">
            <h3 class="text-lg font-semibold">Pre-loaded Results (SSR)</h3>
            <UBadge :label="`${asyncSearchData?.nbHits || 0} results`" color="primary" />
          </div>
        </template>
        <template #body>
          <div v-if="asyncSearchData?.hits?.length" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <UPageCard
    ...
    <section>
      ...
      <UPageCard :ui="{ header: 'flex items-center justify-between w-full border-bottom-1 border-muted' }">
        <template #header>
          <h3 class="text-lg font-semibold">Search for Facet Values</h3>
          <UBadge
        ...
        <template #body>
          <div class="space-y-4">
            ...
            <div v-if="facetSearchResult?.facetHits?.length" class="space-y-2">
              <div
              ...
              >
                ...
                <UBadge :label="hit.value" color="primary" variant="soft" />
              </div>
            </div>
            <UAlert
    ...
    <section>
      <h2 class="text-2xl font-semibold text-primary mb-6 flex items-center gap-2">
        <UIcon name="i-ph-sparkle" class="w-6 h-6" />
      ...
      <UPageCard :ui="{ header: 'w-full border-bottom-1 border-muted' }">
        ...
        <template #body>
          <div v-if="recommendResult?.results?.[0]?.hits?.length" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <UPageCard
    ...
    <section>
      ...
      <UPageCard :ui="{ header: 'w-full border-bottom-1 border-muted' }">
        <template #header>
          <h3 class="text-lg font-semibold">Search with TypeScript</h3>
        </template>
        <template #body>
          <div class="space-y-4">
            <UAlert
    ...
    <section>
      <h2 class="text-2xl font-semibold text-primary mb-6 flex items-center gap-2">
        <UIcon name="i-ph-chart-bar" class="w-6 h-6" />
        ...
        <UPageCard>
          <template #body>
            <div class="text-center">
              ...
              <div class="text-sm text-muted">Total Results</div>
            </div>
          </template>
        </UPageCard>
        <UPageCard>
          <template #body>
            <div class="text-center">
              ...
              <div class="text-sm text-muted">Processing Time</div>
            </div>
          </template>
        </UPageCard>
        <UPageCard>
          <template #body>
            <div class="text-center">
              ...
              <div class="text-sm text-muted">Active Index</div>
            </div>
          </template>
        </UPageCard>
      </div>
...
// SSR: Initial search
const { data: ssrSearchData } = await useAsyncData('ssr-search-results', () =>
