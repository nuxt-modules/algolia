---
title: Indexer
description: Automatically index your content from external CMS providers.
---
...
It will automatically register a new server middleware that you can access manually or by using a webhook to trigger automatic indexing after content change. 

::alert{type="info" icon="i-ph-key"}
Remember to create an appropriate `secret` property and add it to the webhook configuration so that the indexing can be triggered correctly. You can test if it works correctly by accessing `/api/indexer?secret=<YOUR_SECRET>`.
::
